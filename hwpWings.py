...
#----------------------------------------------------------------------------------------------------------------
    ...
    def close(self:None):
        '열린 한글파일을 저장하지 않고 닫는다.'
        return self.hwp.XHwpDocuments.Close(isDirty=False)
...
# 필드 조작하기
    ...
    def move_to_field(self, field_Name:str):
    ...
    def delete_field_by_name(self, field_name:str, idx=-1): # 입력된 field_name과 동일한 필드를 제거한다
        '''
        입력된 field_name과 동일한 누름틀필드를 모두 제거한다.
        단, field만 제거하고, field의 text는 모두 남는다.
        '''
...
# 기타 기능
    ...
    def get_pictures(self:None): # hwpx의 모든 그림파일을 추출함
        '한글 파일 내부의 모든 그림을 추출한다.(.hwpx 파일만 적용가능)'
        # Tkinter 설정
        root = Tk()
        ...
        # 바탕화면에 폴더 생성
        extracted_folder_path = os.path.join(desktop_path, "Extracted_HWXP")
        os.makedirs(extracted_folder_path, exist_ok=True)

        # 파일 선택 다이얼로그 열기 (.hwpx와 .hwp 파일을 선택 가능)
        file_path = filedialog.askopenfilename(
            title="Select a .hwpx or .hwp file",
            filetypes=[("HWPX and HWP files", "*.hwpx *.hwp")]
        )

        # 파일이 선택되지 않았을 때 예외 처리
        if not file_path:
            print("No file selected.")
            # Extracted_HWXP 폴더 삭제
        ...
        zip_file_name = file_name.replace('.hwpx', '.zip').replace('.hwp', '.zip')
        zip_path = os.path.join(extracted_folder_path, zip_file_name)
        ...
        # 압축을 푼 디렉토리에서 BinData 폴더 경로 찾기
        bin_data_path = os.path.join(extracted_folder_path, 'BinData')

        # BinData 폴더가 있을 경우에만 처리
        if os.path.exists(bin_data_path):
            # BinData 폴더 이외의 파일 및 폴더 삭제
            ...
            # BinData 폴더 이름을 images로 변경
            images_path = os.path.join(extracted_folder_path, 'images')
            os.rename(bin_data_path, images_path)

            # images 폴더를 바탕화면으로 이동
            final_images_path = os.path.join(desktop_path, 'images')
            shutil.move(images_path, final_images_path)

            # Extracted_HWXP 폴더 삭제
            shutil.rmtree(extracted_folder_path)
            print(f'Extracted_HWXP deleted. Images folder moved to {final_images_path}')
        else:
            print("BinData folder not found.")
            # Extracted_HWXP 폴더 삭제
            if os.path.exists(extracted_folder_path):
                shutil.rmtree(extracted_folder_path)
    ...
    def hwp_to_hwpx(self, paths: list): # hwp 파일을 hwpx파일로 대량 변환합니다.
        """
        HWP 파일 목록을 받아서 HWPX 형식으로 변환하여 지정된 폴더에 저장하는 메서드.
        :param paths: 변환할 HWP 파일의 경로 목록
        """      
        # Tkinter 초기화 (GUI 창 숨김)
        root = Tk()
        root.withdraw()

        # 저장 경로 설정 (사용자에게 폴더 선택 대화 상자 표시)
        ...
        for file_path in paths:
            ...
            # 저장할 파일 이름 설정 (확장자를 제외한 파일명)
            base_name = os.path.splitext(os.path.basename(file_path))[0]
            output_file_path = os.path.join(output_dir, f"{base_name}.hwpx")
            ...
            # 파일 저장 설정
            self.hwp.HAction.GetDefault("FileSaveAs_S", self.hwp.HParameterSet.HFileOpenSave.HSet)
            self.hwp.HParameterSet.HFileOpenSave.HSet.SetItem("FileName", output_file_path)
            self.hwp.HParameterSet.HFileOpenSave.HSet.SetItem("Format", "HWPX")
            self.hwp.HAction.Execute("FileSaveAs_S", self.hwp.HParameterSet.HFileOpenSave.HSet)
    ...
    def set_font_style(self, fontstyle: str = "굴림"): # 블럭으로 잡은 글자의 글꼴을 변경한다
        # 문자 모양 설정을 초기화합니다.
    ...
    def set_font_size(self, height : int = 10): # 블럭으로 잡은 글자의 크기를 변경한다
        # 문자 모양 설정을 초기화합니다.
        self.hwp.HAction.GetDefault("CharShape", self.hwp.HParameterSet.HCharShape.HSet)
